...
for transcript in list_transcripts :
    ...
    if average_gene_density != 0 :
        ...
        for sliding_w_n in range(0,len( elongation_region_part),3) :    #sliding window using increments of 3 nts
            ...
            for number in range(0,len(amino_window)) :
...
list_codons = []
list_amino_acids 		= amino_enrichment_dict.keys()
...
for amino2 in list_amino_acids :
    ...
    for number in range(0,60) :
        if amino_enrichment_dict[amino2][number][0]  != 0 :
            ...
            list_data.append(0)
    outfile.write(  "\n")
    rust_expected.append(np.mean(codon_enrichment_expected_dict[amino2]))
...
try:
    ...
    shutil.move( tmp_fig_file_name_png,os.path.join(str(argv[5]), 'RUST_amino_metafootprint.png') )

    tmp_fig_file_name_png = "%s.svg"%tmp_fig_file_name
    plt.savefig("%s"%tmp_fig_file_name_png)
    shutil.move( tmp_fig_file_name_png,os.path.join(str(argv[5]), 'RUST_amino_metafootprint.svg') )
    plt.clf()

        
    html = '<p><img alt="RUST amino metafootprint" src="RUST_amino_metafootprint.png" border="1"><br></p>'
